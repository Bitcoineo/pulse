...
export default function MetricCard({ label, value, change, invertChange }: Props) {
  ...
  return (
    <div className="rounded-xl border-l-4 border-l-orange-500 border border-stone-100 bg-white p-6 hover:shadow-md hover:-translate-y-0.5 transition-all">
      <p className="text-sm font-medium text-stone-500">{label}</p>
      <p className="mt-1 text-3xl font-bold text-stone-900 font-mono">{value}</p>
      {!isNeutral && (
