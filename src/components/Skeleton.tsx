export function SkeletonCard() {
  return (
    <div className="rounded-xl border border-stone-100 bg-white p-6 animate-pulse">
...
export function SkeletonChart({ height = "h-[300px]" }: { height?: string }) {
  return (
    <div className={`rounded-xl border border-stone-100 bg-white p-6 animate-pulse ${height}`}>
      <div className="h-4 w-40 rounded bg-stone-200 mb-4" />
      ...
      <div className="space-y-3">
        {Array.from({ length: rows }).map((_, i) => (
          <div key={i} className="flex items-center gap-3">
            <div className="h-3 w-6 rounded bg-stone-200" />
